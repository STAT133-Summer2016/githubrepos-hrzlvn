...
simMH = function(n, k, switch){
  sample_space = 1:n
  selected = new_sample(sample_space, 1)
  grail = new_sample(sample_space, 1)
  set1 = setdiff(sample_space, grail)
  open_set = setdiff(set1, selected)
  open = new_sample(open_set, k)
  set3 = setdiff(sample_space, open)
  set4 = setdiff(set3, selected)
  switched = new_sample(set4, 1)
...
estimatePi = function(num){
  x = replicate(num, simulation())
  prob = sum(x)/length(x)
  return(4*prob)
}
```
