...
mreviews <- mpage %>%
  html_nodes(xpath = '//*[@class="innerBubble"]/*[@class="wrap"]')
...
ratings = mreviews %>% 
  html_nodes(xpath="//div[@class='wrap']/*[@class='rating reviewItemInline']/span/img") %>% 
  html_attr(name = "alt") %>% 
  str_split(" ") %>% 
  sapply(get_info)

dates = mreviews %>% 
  html_nodes(".ratingDate.relativeDate") %>% 
  html_attr("title") %>% 
  mdy()
dates
...
info = data.frame(titles, ratings, dates, reviews)
info
