...
df = read_csv("billboard.csv") %>% 
  ...
  #2: rename Week variable
  df$Week = str_replace_all(df$Week, "^x", "") %>% str_replace_all("(st)*(nd)*(rd)*(th)*.week$", "")
  
  #6: rearrange Artists' names
  mutate(df, artist.inverted = str_replace_all(artist.inverted, "(.*), (.*)", "\\2 \\1"))
  
  #4: change the Week variable into absolute presentation
  df = mutate(df, Week = ymd(date.entered) + dweeks(as.numeric(Week) - 1))
  
  #3: Sunday is the day that update, with no exception.
  unique(c(wday(df$date.entered), wday(df$date.peaked)))
  
  #5: two songs on board at the same week
  destinyschild = na.omit(unique(filter(df, track == "Independent Women Part I" | track == "Jumpin' Jumpin'")))
  week_vector = sort(destinyschild$Week)
  sum(as.integer(duplicated(week_vector)))
  

  #7:
  df <- read_csv("billboard.csv")
  edited_df = gather(df, key = week, value = rank, -year, -artist.inverted, -track, -time, -genre, -date.entered, -date.peaked)
  edited_df$week = str_replace_all(edited_df$week, "^x", "") %>% str_replace_all("(st)*(nd)*(rd)*(th)*.week$", "")
  
  edited_df = transform(edited_df,song_id=as.numeric(factor(track)))
  
  songs = select(edited_df, artist.inverted, track, year, time, song_id)
  rankings = select(edited_df, song_id, date = date.entered, week, rank)
