...
```{r}
# Problem 1
num = str_pad(1:100, 3, side = "left", pad = "0")
files100 = str_c("plot", num, ".png")
files100
```
...
```{r}
length(str_subset(states, "[Ii]"))
```

2. How many states do not contain the letter `a`?
```{r}
length(states) - length(str_subset(states, "[Aa]"))
```

3. Which states contain the letter `j`?
```{r}
length(unique(c(str_subset(states, "j"), str_subset(states, "J"))))
```
...
```{r}
intersect(str_subset(states, "^W"), str_subset(states, "[^aeiou]$"))
```
...
dna_to_aa = function(sequence){
  ana_list = c()
  if(str_detect(sequence, "[^AGCT]")){
    ana_list = "Invalid DNA sequence"
  }
  else{
    codons = read_csv("codons")
    convertion = codons[["amino_acid"]]
    names(convertion) = codons[["dna_triplet"]]
    sequence = strsplit(sequence, "(?<=.{3})", perl = TRUE)[[1]] #split into 3-char-group
    for(i in sequence){
      ana = convertion[i]
      ana_list = str_c(ana_list, ana, sep = "")
    }
  }
  ana_list
}
